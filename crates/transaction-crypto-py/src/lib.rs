...
fn public_from_seed<'py>(py: Python<'py>, seed: &[u8]) -> PyResult<Bound<'py, PyBytes>> {
    let public = public_key_from_seed(seed).map_err(map_err)?;
    Ok(PyBytes::new(py, &public))
}

/// Derives the compact 32-byte Quip account id from serialized H3 public bytes.
#[pyfunction]
fn account_id_from_public<'py>(py: Python<'py>, public: &[u8]) -> Bound<'py, PyBytes> {
    PyBytes::new(py, &account_id_from_public_bytes(public))
}
...
fn sign_payload_from_seed<'py>(
    ...
    let envelope = core_sign_payload_from_seed(seed, payload).map_err(map_err)?;
...
impl HybridSigner {
    ...
    #[staticmethod]
    fn from_mnemonic(secret_uri: &str) -> PyResult<Self> {
        let seed = master_seed_from_secret_uri(secret_uri).map_err(map_err)?;
        Self::from_seed(&seed[..])
    }
    ...
    fn sign<'py>(&self, py: Python<'py>, payload: &[u8]) -> PyResult<Bound<'py, PyBytes>> {
        let envelope = core_sign_payload_from_seed(&self.seed[..], payload).map_err(map_err)?;
        Ok(PyBytes::new(py, &envelope.encode_envelope()))
    }
}
