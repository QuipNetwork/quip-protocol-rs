...
- Do not replace archive-node or indexer workflows for deep historical
  analytics.
- Do not rename existing `WinningSolutions` storage in this changeset.
...
Keep the current block-number keyed qblock storage for compatibility:
...
No qblock index migration is needed.

The deployed network will be recreated with the new schema after these fixes, so
historical `WinningSolutions` entries do not need to be backfilled into the new
ordinal qblock index. This also removes the need to investigate a single-block
or multi-block migration for qblock index backfill in this changeset.
