...
impl FixedHybridEncoding for Ed25519MlDsa44 {
    ...
    fn compose_public_key(
        ...
        pk_bytes[..ED_PK_LEN].copy_from_slice(classical.as_ref());
        pk_bytes[ED_PK_LEN..].copy_from_slice(pq.as_ref());
        PublicKey(pk_bytes)
    }
    ...
    fn compose_signature(
        ...
        sig[..ED_SIG_LEN].copy_from_slice(classical.as_ref());
        sig[ED_SIG_LEN..].copy_from_slice(pq.as_ref());
        Signature(sig)
    }

    fn split_public_key(pk: &Self::PublicKey) -> (&[u8], &[u8]) {
        (&pk.0[..ED_PK_LEN], &pk.0[ED_PK_LEN..])
    }
    ...
    fn split_signature(sig: &Self::Signature) -> (&[u8], &[u8]) {
        (&sig.0[..ED_SIG_LEN], &sig.0[ED_SIG_LEN..])
    }
}
